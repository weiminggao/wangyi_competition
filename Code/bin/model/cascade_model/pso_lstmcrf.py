...
def ps_lstmcrf_model(word, postag, p, s, word_embedding_size, postag_embedding_size, p_embedding_size, \
                     word_embeding, postag_embedding, p_embedding, sequence_length):
    #embedding
    ...
    with tf.name_scope('p_attention'):	 
        p_attention_m = tf.get_variable('p_attention_M', shape = [word_embedding_size[1] + 2 * pos_embedding_size[1]], p_embedding_size[1])
        p_attention_m = tf.tile(tf.expand_dims(p_attention_m, 0), [198, 1, 1])		
        ...
        p_wmr = p_wmr + p_bias
        p_diag = tf.reshape(p_wmr, [198, -1])
        p_diag = tf.nn.softmax(p_diag)
        p_diag = tf.reshape(p_diag, [198, -1, 1])
        p_representation = tf.multiply(input_representation, p_diag)
        
    #s_attension_diag
    with tf.name_scope('s_attention'):
        s_attention_m = tf.get_variable('p_attention_M', shape = [word_embedding_size[1] + 2 * pos_embedding_size[1]], p_embedding_size[1])
        s_attention_m = tf.tile(tf.expand_dims(s_attention_m, 0), [198, 1, 1])		
        s_wm = tf.matmul(p_representation, s_attention_m)		
        s_transpose = tf.transpose(tf.expand_dims(s_embed, 1), [0, 2, 1])
        s_wmr = tf.matmul(s_wm, s_transpose)
        s_bias = tf.get_variable('p_bias', shape = [1])
        s_wmr = s_wmr + s_bias
        s_diag = tf.reshape(s_wmr, [198, -1])
        s_diag = tf.nn.softmax(s_diag)
        s_diag = tf.reshape(s_diag, [198, -1, 1])
    
    #bi_lstm
    with tf.name_scope('bi_lstm'):
        lstm_input = tf.multiply(s_representation, s_diag)
        cell_fw = tf.contrib.rnn.LSTMCell(512)
        cell_bw = tf.contrib.rnn.LSTMCell(512)
        (output_fw, output_bw), _ = tf.nn.bidirectional_dynamic_rnn(cell_fw, cell_bw, lstm_input, sequence_length = sequence_length)
